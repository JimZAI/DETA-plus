...
def print_res(array, name, file=None, prec=4, mult=1):
    ...
    stat_string = ("test {:s}: {:0.%df} +/- {:0.%df}" % (prec, prec)).format(
        name, mean, conf
    )
    print(stat_string)
    if file is not None:
...
def process_copies(embeddings, labels, args):
    ...
    test_embeddings = embeddings.view(
        args["data.test_query"] * args["data.test_way"], n_copy, -1
    ).mean(dim=1)
...
def agreement(preds):
    ...
    for i in range(n_preds):
        for j in range(i, n_preds):
            mat[i, j] = mat[j, i] = (preds[i] == preds[j]).astype("float").mean()
    return mat
