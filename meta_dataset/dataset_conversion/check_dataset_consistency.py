...
flags.DEFINE_string(
    ...
    None,
    "The name of a directory " "containing .tfrecords files and a dataset_spec.json.",
)
flags.DEFINE_string(
    "dataset_spec_file",
    None,
    "Optional, the name and path to "
...
flags.DEFINE_string(
    "label_field_name",
    "label",
    "The name of the field, " "inside each tf.Example, that represents the label.",
)
...
def main(argv):
    ...
    if FLAGS.dataset_spec_file is None:
        dataset_spec = dataset_spec_lib.load_dataset_spec(FLAGS.dataset_records_path)
    else:
    ...
    if dataset_spec.name != dir_name:
        ...
        logging.error(
            ...
            "the name of the directory containing it (%s)",
            dataset_spec.name,
            dir_name,
        )
    ...
    for class_id, filename in enumerate(expected_filenames):
        ...
        try:
            count, labels = get_count_and_labels(full_filepath, FLAGS.label_field_name)
    ...
    if num_failed_checks:
        raise ValueError(
            "%d checks failed. See the error-level logs." % num_failed_checks
        )
