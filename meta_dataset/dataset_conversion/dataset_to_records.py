...
tf.flags.DEFINE_string(
    "omniglot_data_root", "", "Path to the root of the omniglot data."
)

tf.flags.DEFINE_string(
    "aircraft_data_root", "", "Path to the root of the FGVC-Aircraft Benchmark."
)

tf.flags.DEFINE_string(
    "cu_birds_data_root", "", "Path to the root of the CU-Birds dataset."
)

tf.flags.DEFINE_string(
    "dtd_data_root", "", "Path to the root of the Describable Textures Dataset."
)

tf.flags.DEFINE_string(
    "quickdraw_data_root", "", "Path to the root of the quickdraw data."
)

tf.flags.DEFINE_string("fungi_data_root", "", "Path to the root of the fungi data.")

tf.flags.DEFINE_string(
    "vgg_flower_data_root", "", "Path to the root of the VGG Flower data."
)

tf.flags.DEFINE_string(
    "traffic_sign_data_root", "", "Path to the root of the Traffic Sign dataset."
)
...
tf.flags.DEFINE_string(
    "mini_imagenet_data_root", "", "Path to the root of the MiniImageNet data."
)

# Output flags.
tf.flags.DEFINE_string(
    "records_root", "", "The root directory storing all tf.Records of datasets."
)

tf.flags.DEFINE_string(
    "splits_root", "", "The root directory storing the splits of datasets."
)
...
def make_example(features):
    ...
    feature_dict = dict(
        (key, feature_fns[feature_type](value)) for key, feature_type, value in features
    )
...
def write_example(
    data_bytes, class_label, writer, input_key="image", label_key="label"
):
    ...
    example = make_example(
        [(input_key, "bytes", [data_bytes]), (label_key, "int64", [class_label])]
    )
...
def gen_rand_split_inds(num_train_classes, num_valid_classes, num_test_classes):
    ...
    trainval_inds = np.random.choice(num_classes, num_trainval_classes, replace=False)
...
def write_tfrecord_from_image_files(
    class_files,
    class_label,
    output_path,
    invert_img=False,
...
def write_tfrecord_from_directory(
    class_directory,
    class_label,
    output_path,
    invert_img=False,
    ...
    written_images_count = write_tfrecord_from_image_files(
        class_files, class_label, output_path, invert_img, skip_on_error=skip_on_error
    )
...
class DatasetConverter(object):
    ...
    def __init__(
        ...
        if self.split_file is None:
            self.split_file = os.path.join(
                FLAGS.splits_root, "{}_splits.json".format(self.name)
            )
        ...
        if tf.io.gfile.exists(self.split_file):
            with tf.io.gfile.GFile(self.split_file, "r") as f:
                try:
                    ...
                    logging.info(
                        "Unsuccessful: file exists, but loading failed. %s",
    ...
    def get_splits(self, force_create=False):
        ...
        logging.info(
            "Saving new splits for dataset %s at %s...", self.name, self.split_file
        )
...
class OmniglotConverter(DatasetConverter):
    ...
    def parse_split_data(self, split, alphabets, alphabets_path):
        ...
        for alphabet_folder_name in alphabets:
            ...
            for char_folder_name in sorted(tf.io.gfile.listdir(alphabet_path)):
                ...
                self.class_names[class_label] = "{}-{}".format(
                    alphabet_folder_name, char_folder_name
                )
    ...
    def create_dataset_specification_and_records(self):
        ...
        self.parse_split_data(
            learning_spec.Split.TRAIN, training_alphabets, data_path_trainval
        )
        self.parse_split_data(
            learning_spec.Split.VALID, validation_alphabets, data_path_trainval
        )
        self.parse_split_data(learning_spec.Split.TEST, test_alphabets, data_path_test)
...
class QuickdrawConverter(DatasetConverter):
    ...
    def parse_split_data(self, split, split_class_names):
        ...
        for class_name in split_class_names:
            ...
            # Create and write the tf.Record of the examples of this class.
            num_imgs = write_tfrecord_from_npy_single_channel(
...
class CUBirdsConverter(DatasetConverter):
    ...
    def create_splits(self):
        ...
        with tf.io.gfile.GFile(os.path.join(self.data_root, "classes.txt"), "r") as f:
            class_names = []
            for lines in f:
    ...
    def create_dataset_specification_and_records(self):
        ...
        for class_id, class_label in enumerate(all_classes):
            logging.info(
                "Creating record for class ID %d (%s)...", class_id, class_label
            )
            class_records_path = os.path.join(
            ...
            self.images_per_class[class_id] = len(tf.io.gfile.listdir(class_directory))
            write_tfrecord_from_directory(class_directory, class_id, class_records_path)
...
class DTDConverter(DatasetConverter):
    ...
    def create_dataset_specification_and_records(self):
        ...
        for class_id, class_name in enumerate(all_classes):
            logging.info(
                "Creating record for class ID %d (%s)...", class_id, class_name
            )
...
class AircraftConverter(DatasetConverter):
    ...
    def create_dataset_specification_and_records(self):
        ...
        variant_trainval_path = os.path.join(
            self.data_root, "data", "images_variant_trainval.txt"
        )
        ...
        variant_test_path = os.path.join(
            self.data_root, "data", "images_variant_test.txt"
        )
        ...
        for class_id, class_name in enumerate(all_classes):
            ...
            class_files = [
                os.path.join(
                    self.data_root, "data", "images", "{}.jpg".format(filename)
                )
...
class TrafficSignConverter(DatasetConverter):
    ...
    def create_splits(self):
        ...
        splits = {
            ...
            "valid": [],
            "test": [
                "%02d.%s" % (i, class_names[i]) for i in range(self.NUM_TEST_CLASSES)
            ],
        }
        return splits
    ...
    def create_dataset_specification_and_records(self):
        ...
        for class_id, class_label in enumerate(all_classes):
            ...
            class_directory = os.path.join(
                self.data_root, "Final_Training", "Images", "{:05d}".format(class_id)
            )
            class_records_path = os.path.join(
                self.records_path, self.dataset_spec.file_pattern.format(class_id)
            )
...
class MSCOCOConverter(DatasetConverter):
    ...
    def __init__(
        ...
        super(MSCOCOConverter, self).__init__(name, data_root, records_path, split_file)
    ...
    def create_dataset_specification_and_records(self):
        ...
        for i, annotation in enumerate(self.coco_instance_annotations):
            ...
            write_example(
                image_crop_bytes.getvalue(), class_id, class_tf_record_writers[class_id]
            )
...
class ImageNetConverter(DatasetConverter):
    ...
    def _create_data_spec(self, train_split_only=False):
        ...
        for other_dataset in ("Caltech101", "Caltech256", "CUBirds"):
            duplicates_file = os.path.join(
                AUX_DATA_PATH, "ImageNet_{}_duplicates.txt".format(other_dataset)
            )
        ...
        if not ilsvrc_2012_num_leaf_images_path:
            ilsvrc_2012_num_leaf_images_path = os.path.join(
                self.records_path, "num_leaf_images.json"
            )
    ...
    def _get_synset_ids(self, split):
        ...
        return sorted(
            [
                synset.wn_id
                for synset in imagenet_specification.get_leaves(
    ...
    def create_dataset_specification_and_records(self):
        ...
        set_of_directories = set(
            entry
            for entry in tf.io.gfile.listdir(self.data_root)
    ...
    def create_dataset_specification_and_records(self):
        ...
        for class_id, class_label in enumerate(all_classes):
            ...
            if category_id not in class_filepaths:
                raise ValueError(
                    "class_filepaths does not contain paths to any "
...
class MiniImageNetConverter(DatasetConverter):
    ...
    def create_dataset_specification_and_records(self):
        ...
        for classes, split in zip(
            [train_classes, valid_classes, test_classes], ["train", "val", "test"]
        ):
            path = os.path.join(
                self.data_root, "mini-imagenet-cache-{}.pkl".format(split)
            )
            with tf.io.gfile.GFile(path, "rb") as f:
                data = pkl.load(f)
            ...
            for class_id, class_name in zip(classes, names):
                ...
                class_records_path = os.path.join(
                    self.records_path, self.file_pattern.format(class_id)
                )
                self.class_names[class_id] = class_name
