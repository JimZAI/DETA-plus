...
tf.flags.DEFINE_string(
    "all_experiments_root", "", "The overall experiments directory root."
)
...
tf.flags.DEFINE_string(
    "description",
    "best",
    "The description for the output. The output will "
...
tf.flags.DEFINE_string(
    ...
    "",
    "The comma-separated names of the "
...
tf.flags.DEFINE_enum(
    ...
    "",
    ["", "scratch", "imagenet"],
    "The name of a  pretrained_source to " "restrict to for model selection.",
)

tf.flags.DEFINE_integer(
    "smooth_window",
    1,
    "rolling average window to be "
...
def get_value_from_params_dir(params_dir, param_names):
    ...
    for param_name in param_names:
        try:
            try:
                return _load_params(
                    param_name, os.path.join(params_dir, "params.json"), json.load, "r"
                )
            ...
            try:
                return _load_params(
                    param_name, os.path.join(params_dir, "params.pkl"), pkl.load, "rb"
                )
            except tf.errors.NotFoundError:
...
def get_paths_to_events(
    ...
    def keep_variant(name):
        ...
        valid_architecture = (
            not restrict_to_architectures or architecture in restrict_to_architectures
        )
        ...
        valid_pretrained_source = (
            not restrict_to_pretrained_source
            or pretrained_source == restrict_to_pretrained_source
        )
        ...
        return valid_architecture and valid_pretrained_source and valid_variant_name

    variant_names = [
        fname
        for fname in tf.io.gfile.listdir(summary_dir)
    ...
    if not variant_names:
        raise ValueError(
            "Found no subdirectories in {}. Was expecting a "
    ...
    for variant_path, variant_name in zip(variant_paths, variant_names):
        event_filenames = [
            f_name
            for f_name in tf.io.gfile.listdir(variant_path)
        ...
        if len(event_filenames) < 1:
            ...
            logging.info(
                ...
                "in directory %s. Instead, found %d.",
                variant_path,
                len(event_filenames),
            )
            continue
...
def moving_average(x, smooth_window):
    ...
    x = np.pad(
        x, (smooth_window // 2, smooth_window - 1 - (smooth_window // 2)), "reflect"
    )
...
def extract_best_from_event_file(event_path, smooth_window, log_details=False):
    ...
    if not valid_accs:
        ...
        tf.logging.info(
            "Did not find any validation accuracy tags ({}) in event_path {}".format(
                " or ".join(VALIDATION_ACCURACY_TAGS), event_path
            )
        )
        return 0, 0
    if smooth_window > 1:
    ...
    if log_details:
        tf.logging.info(
            "Successfully read event_path {} with best_acc {}".format(
                event_path, best_acc
            )
        )
...
def extract_best_from_variant(event_paths, smooth_window):
    ...
    for event_path in event_paths:
        best_acc_, best_step_ = extract_best_from_event_file(event_path, smooth_window)
...
def main(argv):
    ...
    for root_experiment_dir in experiment_paths:
        ...
        logging.info(
            "%sSelecting the best variant for: %s%s.%s",
            stars_string,
            root_experiment_dir,
            architecture_string,
            stars_string,
        ...
        output_path_pklz = os.path.join(
            root_experiment_dir, "{}.pklz".format(description)
        )
        ...
        output_path_txt = os.path.join(
            root_experiment_dir, "{}.txt".format(description)
        )
        ...
        logging.info(
            ...
            "Just wrote this info to %s and %s",
            best_variant,
            best_valid_acc,
            best_step,
            output_path_pklz,
