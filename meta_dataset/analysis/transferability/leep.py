...
def compute_leep(source_predictions, target_onehot_labels):
    ...
    if target_onehot_labels.shape[0] != batch_size:
        raise ValueError(
            "`source_predictions` and `target_onehot_labels` must "
    ...
    log_per_example_full_joint = tf.einsum(
        "iz,iy->iyz", source_predictions, target_onehot_labels
    )

    # Workaround for one-hot indexing in log space.
    log_per_example_full_joint = tf.where(
    ...
    log_full_joint = tf.math.reduce_logsumexp(
        log_per_example_full_joint, axis=0
    ...
    log_full_target_marginal = tf.math.reduce_logsumexp(
        source_predictions, axis=0
    ) - tf.math.log(batch_size)

    # p_model(y | z) for source labels `y`, conditioning on target labels `z`.
    log_full_conditional = log_full_joint - tf.expand_dims(
        log_full_target_marginal, axis=0
    )

    # p_model(y = y_i | z) for datapoint `i`.
    log_predicted_conditional = tf.einsum(
        "iy,yz->iz", target_onehot_labels, log_full_conditional
    )
