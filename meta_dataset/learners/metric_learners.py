...
class MetricLearner(learner_base.EpisodicLearner):
    ...
    def compute_logits_for_episode(
        self, local_support_embeddings, local_query_embeddings, data
    ):
        all_support_labels = distribute_utils.aggregate(data.onehot_support_labels)
        all_support_embeddings = distribute_utils.aggregate(local_support_embeddings)
    ...
    def compute_logits(
        self, support_embeddings, query_embeddings, onehot_support_labels
    ):
...
class CrossTransformerLearner(MetricLearner):
    ...
    def compute_logits_for_episode(self, support_embeddings, query_embeddings, data):
        ...
        onehot_support_labels = distribute_utils.aggregate(data.onehot_support_labels)
        ...
        if self.rematerialize:
            distances = self._get_dist_rematerialize(
                query_queries, query_values, support_keys, support_values, labels
            )
        else:
            distances = self._get_dist(
                query_queries, query_values, support_keys, support_values, labels
            )
    ...
    @tf.custom_gradient
    def _get_dist_rematerialize(
        self, query_queries, query_values, support_keys, support_values, labels
    ):
        ...
        def fwd_fn(
            ...
            initial = (
                0,
                tf.zeros(
                    [tf.reduce_max(labels) + 1, zero_dim], dtype=query_queries_fwd.dtype
                ),
            )

            def loop_body(idx, dist):
                dist_new = self._get_dist(
                    query_queries_fwd[idx : idx + 1],
        ...
        def grad_fn(dy):
            ...
            initial = (
                0,
                tf.zeros(tf.shape(query_queries)[1:], dtype=dy.dtype)[tf.newaxis, :][
                    :zero_dim
                ],
                tf.zeros(tf.shape(query_values)[1:], dtype=dy.dtype)[tf.newaxis, :][
                    :zero_dim
                ],
            ...
            def loop_body(idx, qq_grad, qv_grad, sk_grad, sv_grad):
                ...
                grads = tf.gradients(
                    x,
                    [qq, qv, support_keys_id, support_values_id],
        ...
        return (
            fwd_fn(query_queries, query_values, support_keys, support_values, labels),
            grad_fn,
        )
    ...
    def _get_dist(
        ...
        attended_values = self._attend(
            query_queries, support_keys, support_values, labels
        )
        # query_aligned_prototypes: [N_classes, n_query, h_query, w_query, C]
        query_aligned_prototypes = tf.math.unsorted_segment_sum(
            attended_values, labels, tf.reduce_max(labels) + 1
        )

        # (scaled) Euclidean distance
        shp = tf.shape(query_values)
        aligned_dist = tf.square(
            query_values[tf.newaxis, Ellipsis] - query_aligned_prototypes
        )
    ...
    def get_support_set_softmax(self, logits, class_ids):
        ...
        max_logit = tf.math.unsorted_segment_max(
            max_logit, class_ids, tf.reduce_max(class_ids) + 1
        )
        ...
        sum_exp_x = tf.math.unsorted_segment_sum(
            sum_exp_x, class_ids, tf.reduce_max(class_ids) + 1
        )
...
class MatchingNetworkLearner(MetricLearner):
    ...
    def compute_logits(
        ...
        similarities = tf.matmul(query_embeddings, support_embeddings, transpose_b=True)
...
class RelationNetworkLearner(MetricLearner):
    ...
    def compute_logits(
        ...
        relation_pairs = tf.reshape(relation_pairs, [-1] + out_shape + [n_feature * 2])
