...
class BaselineFinetuneLearner(baseline_learners.BaselineLearner):
    ...
    def compute_logits(self, data):
        ...
        (
            ...
            embedding_vars,
            embedding_vars_copy_ops,
        ) = get_embeddings_vars_copy_ops(support_embeddings_var_dict, make_copies)
                ...
                with tf.control_dependencies(
                    ...
                    with tf.control_dependencies([print_op]):
                        ...
                        if self.debug_log:
                            ...
                            query_logits = self._fc_layer(query_embeddings)[
                                :, 0 : data.way
                            ]

            else:
                with tf.control_dependencies(
                    [finetune_op, finetune_loss] + vars_to_finetune
                ):
                    print_op = tf.no_op()
                    if self.debug_log:
                        print_op = tf.print(
                            [
                                ...
                                self.compute_accuracy(
                                    labels=data.onehot_support_labels,
                                    predictions=logits,
                                ),
                                "query accuracy:",
                                self.compute_accuracy(
                                    labels=data.onehot_query_labels,
                                    predictions=query_logits,
                                ),
                            ]
                        )
    ...
    def _get_finetune_op(
        ...
        if self.finetune_with_adam:
            ...
            update_ops = gradient_descent_step(
                finetune_loss, vars_to_finetune, True, False, self.finetune_lr
            )["update_ops"]
...
class MAMLLearner(OptimizationLearner):
    ...
    def forward_pass(self, data):
        ...
        def _body(step, *args):
            ...
            support_logits = (
                tf.matmul(support_embeddings, updated_fc_weights) + updated_fc_bias
            )

            support_logits = support_logits[:, 0 : data.way]
            loss = tf.losses.softmax_cross_entropy(
                data.onehot_support_labels, support_logits
            )
            ...
            with tf.control_dependencies([print_op]):
                updated_embedding_vars = gradient_descent_step(
                    loss,
                    updated_embedding_vars,
                    self.first_order,
                    self.adapt_batch_norm,
                    self.alpha,
                    False,
                )["updated_vars"]
                updated_fc_vars = gradient_descent_step(
                    loss,
                    updated_fc_vars,
                    self.first_order,
                    self.adapt_batch_norm,
                ...
                step = step + 1
            return tuple([step] + list(updated_embedding_vars) + list(updated_fc_vars))
        ...
        query_logits = (
            tf.matmul(query_embeddings, updated_fc_weights) + updated_fc_bias
        )[:, 0 : data.way]
