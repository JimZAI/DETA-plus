...
class MockEpisode(
    collections.namedtuple(
        "MockEpisode", "support_images, query_images, " "support_labels, query_labels"
    )
):
    @property
    def way(self):
...
class TestLearner(tf.test.TestCase):
    ...
    def set_up_learner(self):
        ...
        if issubclass(self.learner_cls, experimental_learner_base.ExperimentalLearner):
    ...
    def testLearnerInitRandomAccuracy(self):
        ...
        with self.session():
            ...
            self.assertLess(accuracy_value, 1.0 / data.way + epsilon)
