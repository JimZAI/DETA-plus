...
def compute_prototypes(embeddings, onehot_labels):
    ...
    class_sums = tf.einsum(
        "ij,i{0}->j{0}".format(embedding_indices), onehot_labels, embeddings
    )
...
class MetricLearner(learner_base.ExperimentalEpisodicLearner):
    ...
    def compute_class_distances(
        self, support_embeddings, onehot_support_labels, query_embeddings
    ):
...
class MatchingNetwork(MetricLearner):
    ...
    def __init__(self, distance_metric=partial_cosine_distance, **kwargs):
        ...
        Args:
          ...
          **kwargs: Arguments common to all `MetricLearner`s.
        """
        super(MatchingNetwork, self).__init__(distance_metric=distance_metric, **kwargs)
    ...
    def compute_class_distances(
        ...
        similarities = 1 - self.distance_metric(query_embeddings, support_embeddings)
        attention = tf.nn.softmax(similarities)

        # [num_query_images, way]
        probs = tf.matmul(attention, tf.cast(onehot_support_labels, dtype=tf.float32))
...
class RelationNetwork(MetricLearner):
    ...
    def __init__(
        self, relation_module_cls=reparameterizable_backbones.RelationModule, **kwargs
    ):
        ...
        if not isinstance(
            self.embedding_fn, reparameterizable_backbones.RelationNetConvNet
        ):
            raise ValueError(
                "`embedding_fn` for `RelationNetwork` must be a "
    ...
    def compute_class_distances(
        ...
        relation_pairs = tf.concat(
            (prototype_extended, query_extended), len(out_shape) + 1
        )

        # relation_pairs.shape.as_list()[-3:] == [-1] + out_shape + [num_features*2]
        relation_pairs = tf.reshape(
            relation_pairs, [-1] + out_shape[:-1] + [num_features * 2]
        )
