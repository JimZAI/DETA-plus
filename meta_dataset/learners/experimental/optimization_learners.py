...
def adam(learning_rate, b1=0.9, b2=0.999, eps=1e-8):
    ...
    def update(i, grad, state):
        ...
        x, m, v = state
        m = (1.0 - b1) * grad + b1 * m  # First  moment estimate.
        v = (1.0 - b2) * (grad**2.0) + b2 * v  # Second moment estimate.
        mhat = m / (1.0 - b1 ** (i + 1.0))  # Bias correction.
        vhat = v / (1.0 - b2 ** (i + 1.0))
...
ForwardPass = collections.namedtuple(
    "ForwardPass",
    (
        "embeddings",
        "predictions",
...
Adaptation = collections.namedtuple(
    "Adaptation",
    (
...
class ExperimentalOptimizationLearner(learner_base.ExperimentalEpisodicLearner):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert isinstance(
            self.embedding_fn, reparameterizable_base.ReparameterizableModule
        )
    ...
    def detailed_forward_pass(self, data):
        ...
        def _m_step(preupdate_vars, all_embeddings_, all_responsibilities_):
            ...
            means, log_scales, logits = zip(
                *map(
                    reparameterizable_distributions.fit_gaussian_mixture,
                    all_embeddings_,
                    all_responsibilities_,
        ...
        with tf.control_dependencies(episodic_init_ops):
            ...
            num_optimizer_steps = self.num_update_steps + (
                self.additional_evaluation_update_steps if not self.is_training else 0
            )
...
class HeadAndBackboneLearner(ExperimentalOptimizationLearner):
    ...
    def __init__(
        self, head_cls, adapt_head_predicate, episodic_head_init_fn=None, **kwargs
    ):
    ...
    def compute_regularizer(self, onehot_labels, predictions):
        ...
        del onehot_labels
        del predictions
        return tf.reduce_sum(input_tensor=self.embedding_fn.losses) + tf.reduce_sum(
            input_tensor=self.head_fn.losses
        )
    ...
    def trainable_variables(self):
        ...
        if not self._built:
            raise learner_base.NotBuiltError
        return self.embedding_fn.trainable_variables + self.head_fn.trainable_variables
...
def proto_maml_fc_layer_init_fn(
    labels, embeddings, weights, biases, prototype_multiplier
):
    ...
    output_biases = tf.pad(
        ...
        paddings=[[0, tf.shape(input=biases)[0] - tf.shape(input=output_biases)[0]]],
        mode="CONSTANT",
        constant_values=0,
    )
...
class MAML(HeadAndBackboneLearner):
    ...
    def __init__(self, proto_maml_fc_layer_init, zero_fc_layer_init, **kwargs):
        ...
        super(MAML, self).__init__(
            episodic_head_init_fn=(
                proto_maml_fc_layer_init_fn
                if proto_maml_fc_layer_init
                else zero_init_fn
                if zero_fc_layer_init
                else None
            ),
...
class GenerativeClassifier(HeadAndBackboneLearner):
    ...
    def __init__(self, generative_scaling, interpolation_schedule, **kwargs):
        ...
        assert isinstance(
            self.head_fn, reparameterizable_distributions.ReparameterizableClassMixture
        )
        ...
        assert all(
            coef >= 0 for coef in self.gen_disc_interpolation
        ...
        if isinstance(self.head_fn, reparameterizable_distributions.GaussianMixture):
            ...
            assert all(
                coef == 0.0 for coef in self.gen_disc_interpolation[:num_em_steps]
            ) and all(
                coef == 1.0 for coef in self.gen_disc_interpolation[num_em_steps:]
    ...
    def validate_model_independence(self, labels, log_probs, task_parameters):
        ...
        on_gradients = [
            ...
            for on_gradients in [
                tf.gradients(
                    ...
                    xs=task_parameters[
                        i * num_task_parameters : (i + 1) * num_task_parameters
                    ],
                )
                for i in range(1)
            ]
        ...
        return (
            list(itertools.chain.from_iterable(on_gradients)),
