...
OPTIMIZER_KEYWORDS = ("adam", "power", "global_step", "data_dependent_init_done")
...
def is_backbone_variable(variable_name, only_if=lambda x: True):
    ...
    is_embedding_var = any(keyword in variable_name for keyword in EMBEDDING_KEYWORDS)
    ...
    is_optimizer_var = any(keyword in variable_name for keyword in OPTIMIZER_KEYWORDS)

    return (
        only_if(variable_name)
        and is_embedding_var
        and not is_head_classifier_var
        and not is_relationnet_var
        and not is_optimizer_var
    )
    ...
    with tf.variable_scope("batch_norm"):
        ...
        if moments is not None:
            ...
            if replica_ctx:
                ...
                x_sum, x_squared_sum, global_batch_size = replica_ctx.all_reduce(
                    "sum", [local_sum, local_squared_sum, batch_size]
                )
        ...
        if use_ema and is_training and moments is None:
            replica_ctx = tf.distribute.get_replica_context()
            mean_upd = tf.assign(
                mean_ema, mean_ema * ema_epsilon + mean * (1.0 - ema_epsilon)
            )
            var_upd = tf.assign(
                var_ema, var_ema * ema_epsilon + var * (1.0 - ema_epsilon)
            )
...
def conv(
    x, conv_size, depth, stride, weight_decay, padding="SAME", rate=1, params=None
):
    ...
    if params is None:
        w_conv = weight_variable(
            [conv_size[0], conv_size[1], input_depth, depth], weight_decay
        )
    else:
...
def _resnet(
    ...
    params_keys, params_vars = [], []
    moments_keys, moments_vars = [], []
    assert max_stride in [
        None,
        4,
        8,
        16,
        32,
    ], "max_stride must be 4, 8, 16, 32, or None"
    with tf.variable_scope(scope, reuse=reuse):
        ...
        def _bottleneck(x, i, depth, stride, params, moments, net_stride=1, net_rate=1):
            ...
            for i in range(blocks[0]):
                with tf.variable_scope("bottleneck_%d" % i):
                    (
                        ...
                        net_stride,
                        net_rate,
                    ) = _bottleneck(x, i, 64, 1, params, moments, net_stride, net_rate)
            ...
            for i in range(blocks[1]):
                with tf.variable_scope("bottleneck_%d" % i):
                    (
                        ...
                        net_stride,
                        net_rate,
                    ) = _bottleneck(x, i, 128, 2, params, moments, net_stride, net_rate)
                    params_keys.extend(bottleneck_params.keys())
                    params_vars.extend(bottleneck_params.values())
                    moments_keys.extend(bottleneck_moments.keys())
                    moments_vars.extend(bottleneck_moments.values())
            ...
            for i in range(blocks[2]):
                with tf.variable_scope("bottleneck_%d" % i):
                    (
                        ...
                        net_stride,
                        net_rate,
                    ) = _bottleneck(x, i, 256, 2, params, moments, net_stride, net_rate)
                    params_keys.extend(bottleneck_params.keys())
                    params_vars.extend(bottleneck_params.values())
                    moments_keys.extend(bottleneck_moments.keys())
                    moments_vars.extend(bottleneck_moments.values())
            ...
            for i in range(blocks[3]):
                with tf.variable_scope("bottleneck_%d" % i):
                    (
                        ...
                        net_stride,
                        net_rate,
                    ) = _bottleneck(x, i, 512, 2, params, moments, net_stride, net_rate)
                    params_keys.extend(bottleneck_params.keys())
                    params_vars.extend(bottleneck_params.values())
                    moments_keys.extend(bottleneck_moments.keys())
                    moments_vars.extend(bottleneck_moments.values())
...
def wide_resnet_block(
    ...
    with tf.variable_scope("conv2"):
        ...
        h_2, conv_params = conv(
            out_2, [3, 3], depth, stride=1, weight_decay=weight_decay, params=params
        )
        params_keys.extend(conv_params.keys())
        params_vars.extend(conv_params.values())
...
def _four_layer_convnet(
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        return_dict = {"embeddings": layer, "params": model_params, "moments": moments}

        return return_dict


@gin.configurable("four_layer_convnet", allowlist=["weight_decay"])
def four_layer_convnet(
    inputs,
    is_training,
    weight_decay,
    params=None,
...
def fully_connected_network(
    ...
    model_params = collections.OrderedDict(zip(model_params_keys, model_params_vars))
    moments = collections.OrderedDict(zip(moments_keys, moments_vars))
    return_dict = {"embeddings": layer, "params": model_params, "moments": moments}
    return return_dict
