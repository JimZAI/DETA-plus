...
def fit_gaussian(embeddings, damping=1e-7, full_covariance=False):
    ...
    if full_covariance:
        ...
        sample_covariance = tf.einsum(
            "ij,ik->kj", centered_embeddings, centered_embeddings
        )  # Outer product.
        ...
        log_variances = tf.math.log(
            sample_variances + damping * tf.ones_like(sample_variances)
        )
        return sample_mean, log_variances


def fit_gaussian_mixture(
    embeddings, responsibilities, damping=1e-7, full_covariance=False
):
    ...
    sample_mean /= tf.reduce_sum(input_tensor=responsibilities, axis=0)[:, tf.newaxis]
    centered_embeddings = embeddings[:, tf.newaxis, :] - sample_mean[tf.newaxis, :, :]

    if full_covariance:
        sample_covariance = tf.einsum(
            "ijk,ijl->ijkl", centered_embeddings, centered_embeddings
        )  # Outer product.
        sample_covariance += damping * tf.eye(dim)  # Positive definiteness.
        weighted_covariance = tf.einsum(
            "ij,ijkl->jkl", responsibilities, sample_covariance
        )
        weighted_covariance /= tf.reduce_sum(input_tensor=responsibilities, axis=0)[
        ...
        sample_variances = (
            avg_x_squared - 2 * avg_x_means + avg_means_squared + damping * tf.ones(dim)
        )
...
class ReparameterizableClassMixture(reparameterizable_base.ReparameterizableModule):
...
class MultivariateNormalDiag(ReparameterizableClassMixture):
    ...
    def episodic_init_ops(self, onehot_labels, embeddings, task_parameters):
        """Perform (data-dependent) initialization operations."""
        del task_parameters
        class_embeddings = learner_base.class_specific_data(
            onehot_labels, embeddings, self.num_components
        )
        ...
        return _assign_pairs(self.locs, locs) + _assign_pairs(
    ...
    def components(self):
        ...
        return [
            tfd.MultivariateNormalDiag(
                loc=loc, scale_diag=tf.math.softplus(log_scale), allow_nan_stats=False
            )
...
class GaussianMixture(ReparameterizableClassMixture):
    ...
    def episodic_init_ops(self, onehot_labels, embeddings):
        ...
        for component_locs, component_log_scales, component_logits in zip(
            ...
            init_ops += _assign_pairs(component_locs, _split_mode_params(self.meta_loc))
            init_ops += _assign_pairs(
                component_log_scales, _split_mode_params(self.meta_log_scale)
            )
    ...
    def __call__(self, embeddings, components=False, class_idx=None):
        ...
        if components:
            return tf.stack(
                [
                    tf.stack([c.log_prob(embeddings) for c in cs.components], axis=-1)
...
class RealNVPModel(DeepDensity):
    ...
    def __init__(
        self, num_coupling_layers, num_hidden_units, num_hidden_layers, **kwargs
    ):
    ...
    def components(self):
        ...
        def _make_shift_and_log_scale_fn(shift_and_log_scale_model, num_masked):
            ...
            def _shift_and_log_scale_fn(x, output_dims, *args, **kwargs):
                ...
                if output_dims != self.num_dims - num_masked:
                    raise ValueError(
                        "Expected {} output_dims, got {}.".format(
        ...
        for j in range(self.num_components):
            ...
            for i, shift_and_log_scale_model in enumerate(
                ...
                bijector = tfb.RealNVP(
                    num_masked=num_masked, shift_and_log_scale_fn=shift_and_log_scale_fn
                )
        ...
        return [
            ...
            for bijectors in class_bijectors
        ]
