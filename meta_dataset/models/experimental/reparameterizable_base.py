...
def is_batch_norm_module(obj):
    return (
        isinstance(obj, tf.keras.layers.BatchNormalization) or "batch_norm" in obj.name
    )
...
class ReparameterizableModule(tf.Module):
    ...
    def reparameterizables(
        self, variable_predicate=is_variable, module_predicate=None, with_path=False
    ):
        ...
        if module_predicate is None:
            ...
            def satisfies_module_predicate(variable_with_path):
                ...
                for path_prefix in path_prefixes(path):
                    ...
                    if isinstance(prefix_module, tf.Module) and module_predicate(
                        prefix_module
                    ):
                        return True
            ...
            filtered_variables = filter(
                satisfies_module_predicate, variables_with_paths
            )
            if with_path:
