...
def assign_variables(variable_mapping):
    ...
    for variable, reference_variable in variable_mapping:
        if tf.executing_eagerly():
            ...
            initializer_ops = [
                variable._initializer_op
            ]  # pylint: disable=protected-access
            if isinstance(reference_variable, tf.Variable):
                initializer_ops += [
                    reference_variable._initializer_op
                ]  # pylint: disable=protected-access
...
def ckpt_parameter_adapter(model, ckpt_path, renamed_variable_generator):
    ...
    replacement_mapping = []
    for new_tensor, old_tensor_name in renamed_variable_generator(model, ckpt_vars):
...
def filter_and_sort_vars(list_of_vars, string):
    try:
        return sorted(
            filter(lambda x: string in x, list_of_vars), key=single_to_double_digits
        )
    except TypeError:
...
def meta_dataset_functional_variable_generator(
    reparameterizable_model, functional_ckpt_vars
):
    ...
    ckpt_projection_vars = filter_and_sort_vars(functional_ckpt_vars, "projection")
    ...
    ckpt_output_layer_vars = filter_and_sort_vars(functional_ckpt_vars, "embedding")
    ...
    ckpt_input_layer_vars = list(
        filter_and_sort_vars(functional_ckpt_vars, "resnet18/conv1")
...
def bit_variable_generator(model, imported_model):
    ...
    imported_name_to_variable = dict((v.name, v) for v in imported_model.variables)
    ...
    for n in ("gamma", "beta") + (
        () if group_norm else ("moving_mean", "moving_variance")
    ):
        name = "{}{}_normalization_{}/{}:0".format(
            output_stage_prefix,
            "group" if group_norm else "batch",
            16
            if (
    ...
    if isinstance(model, reparameterizable_backbones.ResNet18V2) or isinstance(
        model, reparameterizable_backbones.GNResNet18V2
    ):
        for i, (stage, block, layer) in enumerate(
            ...
            for n in ("gamma", "beta") + (
                ...
                pairs += [
                    (name_to_variable[name], imported_name_to_variable[imported_name])
                ]

            if stage > 0 and block == layer == 0:
                name = "stage_{}/block_{}/shortcut_conv/kernel:0".format(stage, block)
                imported_name = imported_base_path + "proj/{}conv2d/kernel:0".format(
                    "standardized_" if group_norm else ""
                )
                pairs += [
                    (name_to_variable[name], imported_name_to_variable[imported_name])
    ...
    elif isinstance(model, reparameterizable_backbones.ResNet50V2) or isinstance(
        model, reparameterizable_backbones.GNResNet50V2
    ):
        conv_id = 1
        batch_norm_id = 0
        for stage, num_blocks in enumerate((3, 4, 6, 3)):
            for block in range(num_blocks):
                for layer in range(3):
                    imported_base_path = "resnet/block{}/unit0{}/{}/".format(
                        stage + 1, block + 1, {0: "a", 1: "b", 2: "c"}[layer]
                    )
                    ...
                    for n in ("gamma", "beta") + (
                        ...
                        name = "stage_{}/block_{}/{}_normalization{}/{}:0".format(
                            ...
                            block,
                            "group" if group_norm else "batch",
                            "_{}".format(batch_norm_id) if batch_norm_id > 0 else "",
                            n,
                        )
                        imported_name = imported_base_path + "{}/{}:0".format(
                            "group_norm" if group_norm else "batch_normalization", n
                ...
                if block == 0:
                    ...
                    imported_name = (
                        "resnet/block{}/unit01/a/proj/{}conv2d/kernel:0".format(
        ...
        raise NotImplementedError(
            "BiT variable mappings are not defined for "
