...
def paired_named_parameter_generators(torch_model, tf_model):
    ...
    for (
        ...
        torch_parameter_names, tf_parameter_names, torch_parameters, tf_parameters
    ):
        ...
        if tf_parameter is None:
            ...
            is_convolutional_kernel = len(torch_parameter.shape) == 4 and (
                "conv" in torch_parameter_name or "downsample" in torch_parameter_name
            )
...
if TORCH_AVAILABLE:
    ...
    class ResNetTest(tf.test.TestCase, parameterized.TestCase):
        ...
        def test_shapes_against_torch_model(self, torch_module_class, tf_module_class):
            ...
            for (
                ...
                tf_parameter,
                np_parameter,
            ) in paired_named_parameter_generators(torch_model, tf_model):

                parameter_names = "lhs: {}; rhs: {}".format(
                    torch_parameter_name, tf_parameter_name
                )
                # pylint: disable=g-assert-in-except
                try:
                    self.assertShapeEqual(
                        np_parameter, tf_parameter, msg=parameter_names
                    )
                except TypeError:
        ...
        @parameterized.named_parameters(*TORCH_TENSORFLOW_PAIRS)
        def test_outputs_against_torch_model(self, torch_module_class, tf_module_class):
            torch_model = torch_module_class(pretrained=False)
            ...
            for (
                ...
                try:
                    model_initialization_ops.append(
                        tf.assign(tf_parameter, np_parameter)
                    )
                except ValueError:
                    model_initialization_ops.append(
