...
class ReparameterizableDense(
    reparameterizable_base.ReparameterizableModule, tf.keras.layers.Dense
):
    pass


class ReparameterizableConv2D(
    reparameterizable_base.ReparameterizableModule, tf.keras.layers.Conv2D
):
    pass


class ReparameterizableBatchNormalization(
    reparameterizable_base.ReparameterizableModule, tf.keras.layers.BatchNormalization
):
...
MODULE_REPARAMETERIZING_PREDICATES = (reparameterizable_base.is_batch_norm_module,)
...
def has_constant_jacobian(layer):
    return any(isinstance(layer, cls) for cls in CONSTANT_JACOBIAN_TRANSFORMS) and (
        not hasattr(layer, "activation")
        or layer.activation is None
        or "linear" in str(layer.activation)
    )


def _get_module_predicate_test_cases(
    module_cls,
    variable_predicate,
    module_predicate,
    valid_module_init_args,
...
def get_module_test_cases(
    reparameterizable_modules=None,
    ...
    for variable_predicate, module_predicate in itertools.product(
        ...
        for module_cls in reparameterizable_modules:
            test_cases += _get_module_predicate_test_cases(
                module_cls,
                variable_predicate,
                module_predicate,
                valid_module_init_args,
...
def _randomized_variables(variables):
    ...
    def rv(size):
        return np.random.normal(scale=0.01, size=size).astype(np.float32)

    randomized = dict(
        (v_ref, tf.Variable(rv(size=v_ref.deref().shape.as_list())))
...
def _init_reference_module(module_cls, module_init_kwargs, paths, variables):
    ...
    for path, variable in zip(paths, variables):
        ...
        reparameterizable_base.corner_case_setattr(descoped_module, path[-1], variable)
...
def _setup_modules(
    ...
    # Reference modules.
    before_reference_module = _init_reference_module(
        module_cls, module_init_kwargs, paths, variables
    )
    after_reference_module = _init_reference_module(
        module_cls, module_init_kwargs, paths, replacement_variables
    )
...
class TestReparameterizableModule(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_swap_and_revert_parameters(
        ...
        with self.session(use_gpu=True) as sess:
            ...
            # The output should conform before variable replacement.
            self.assertAllEqual(before_reparameterization_value, reference_before_value)
    ...
    def test_weight_gradients_after_swap_and_revert(
        ...
        with self.session(use_gpu=True) as sess:
            ...
            for grads in values["before"]:
                for grad, grad_ref in zip(
                    results["before"][grads], reference_before_result
                ):
            ...
            for grads in values["after"]:
                for grad, grad_ref in zip(
                    results["after"][grads], reference_after_result
                ):
                    self.assertAllClose(grad, grad_ref, rtol=1e-05)
    ...
    def test_input_gradients_after_swap_and_revert(
        ...
        # Reference outputs.
        reference_before_result = tf.gradients(before_reference_module(inputs), inputs)
        reference_after_result = tf.gradients(after_reference_module(inputs), inputs)
