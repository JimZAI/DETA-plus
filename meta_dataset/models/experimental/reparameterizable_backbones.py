...
@gin.configurable
class ReparameterizableBackbone(
    tf.keras.Sequential, reparameterizable_base.ReparameterizableModule
):
    ...
    def output_layers(self):
        ...
        if not self.keep_spatial_dims:
            return [tf.keras.layers.Flatten()] + super(
                ReparameterizableSpatialBackbone, self
            ).output_layers
        else:
...
def weight_standardization_replacements(model):
    ...
    if not isinstance(model, ReparameterizableBackbone):
        raise ValueError("`model` must be an instance of `ReparameterizableBackbone`.")
    kernels = filter(
        lambda v: "kernel" in v.name and "output" not in v.name,
...
class GNResNet(ResNet):
    normalization_layer = functools.partial(tfa.layers.GroupNormalization, groups=32)
