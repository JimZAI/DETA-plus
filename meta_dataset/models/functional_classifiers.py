...
def linear_classifier_forward_pass(
    embeddings, w_fc, b_fc, cosine_classifier, cosine_logits_multiplier, use_weight_norm
):
...
def linear_classifier(
    ...
    if use_weight_norm:
        ...
        data_dependent_init_done = tf.get_variable(
            "data_dependent_init_done", initializer=0, dtype=tf.int32, trainable=False
        )

        w_fc = tf.get_variable(
            "w_fc",
            [embedding_dims, num_classes],
        ...
        g = tf.get_variable(
            "g", dtype=tf.float32, initializer=tf.ones([num_classes]), trainable=True
        )
        b_fc = None
        ...
        def _do_data_dependent_init():
            ...
            g_init_value = 1.0 / tf.sqrt(var_init + 1e-10)
        ...
        init_op = tf.cond(
            tf.equal(data_dependent_init_done, 0), _do_data_dependent_init, tf.no_op
        )
        ...
        with tf.control_dependencies([init_op]):
            ...
            logits = linear_classifier_forward_pass(
                embeddings,
                w_fc,
                b_fc,
                cosine_classifier,
        ...
        # Forward pass through the layer defined by w_fc and b_fc.
        logits = linear_classifier_forward_pass(
            embeddings, w_fc, b_fc, cosine_classifier, cosine_logits_multiplier, False
        )
    return logits
