...
flags.DEFINE_multi_string("gin_config", None, "List of paths to the config files.")
flags.DEFINE_multi_string("gin_bindings", None, "List of Gin parameter bindings.")
flags.DEFINE_string(
    "output_dir", "/tmp/cached_episodes/", "Root directory for saving episodes."
)
flags.DEFINE_integer("num_episodes", 600, "Number of episodes to sample.")
flags.DEFINE_string(
    "dataset_name", "omniglot", "Dataset name to create " "episodes from."
)
flags.DEFINE_enum_class(
    "split",
    learning_spec.Split.TEST,
    learning_spec.Split,
    "See learning_spec.Split for " "allowed values.",
)
flags.DEFINE_boolean(
    "ignore_dag_ontology",
    False,
    "If True the dag ontology" " for Imagenet dataset is not used.",
)
flags.DEFINE_boolean(
    "ignore_bilevel_ontology",
    False,
    "If True the bilevel" " sampling for Omniglot dataset is not used.",
)
tf.flags.DEFINE_string(
    "records_root_dir", "", "Root directory containing a subdirectory per dataset."
)
FLAGS = flags.FLAGS
...
def main(unused_argv):
    ...
    use_dag_ontology = (
        FLAGS.dataset_name in ("ilsvrc_2012", "ilsvrc_2012_v2")
    ...
    for episode_number, (episode, _) in enumerate(dataset):
        ...
        images_per_class_dict[os.path.basename(path_train)] = utils.get_label_counts(
            train_labels
        )
        images_per_class_dict[os.path.basename(path_test)] = utils.get_label_counts(
            test_labels
        )
