...
class DumpAndReadEpisodesTest(tf.test.TestCase, parameterized.TestCase):
    def dump_episodes(self, n_episode, n_image_train=5, n_image_test=10):
        ...
        for i in range(n_episode):
            ...
            images_per_class_dict[os.path.basename(test_path)] = utils.get_label_counts(
                test_labels
            )
    ...
    def test_single_dataset(self, n_image=10):
        ...
        for i, (train_ds, test_ds) in enumerate(episodes):
            train_loader = read_episodes.read_episode_as_dataset(record_dir, i, "train")
    ...
    def test_read_as_meta_dataset(self, n_image=10):
        record_dir, episodes = self.dump_episodes(2)
        episode_ds, n_episodes = read_episodes.read_episodes_from_records(record_dir)
        total_episodes = 0
