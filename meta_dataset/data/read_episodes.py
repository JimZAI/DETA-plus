...
def read_episode_as_dataset(episodes_dir, episode_index, split, with_info=False):
...
def read_episodes_from_records(
    episodes_dir, train_suffix=utils.TRAIN_SUFFIX, test_suffix=utils.TEST_SUFFIX
):
...
def read_episodes_from_records_multiple_sources(
    episodes_dir_list, train_suffix=utils.TRAIN_SUFFIX, test_suffix=utils.TEST_SUFFIX
):
    ...
    return tf.data.experimental.sample_from_datasets(dataset_per_dir), num_files_per_dir
...
def read_vtab_as_episode(vtab_key, image_size=224, query_size_limit=500):
    ...
    dataset_instance = data_loader.get_dataset_instance(
        {"dataset": "data.%s" % vtab_key, "data_dir": None}
    )
