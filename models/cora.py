...
class CoRA(nn.Module):
    def __init__(
        self, temperature=0.3, contrast_mode="all", base_temperature=0.07
    ):
        ...
        if is_weight_patch:
            ...
            with torch.no_grad():
                ...
                for w in range(way):
                    ...
                    ].sum(-1) / (K_patch * shot_nums[w] - K_patch + e)
                    w_ng = simscore_ng[
                        point : point + shot_nums[w] * K_patch
                    ].sum(-1) / (N_patch - K_patch * shot_nums[w] + e)
                    if is_softmax:
