...
class CosineConv(nn.Module):
    ...
    def forward(self, x):
        x_normalized = torch.nn.functional.normalize(x, p=2, dim=1, eps=1e-12)
        weight = torch.nn.functional.normalize(self.weight, p=2, dim=1, eps=1e-12)
...
class CheckPointer(object):
    ...
    def restore_model(self, ckpt="last", model=True, optimizer=True, strict=True):
        ...
        return (
            ch.get("epoch", None),
            ch.get("best_val_loss", None),
            ch.get("best_val_acc", None),
        )
    ...
    def restore_out_model(self, ckpt="last", model=True, optimizer=True, strict=True):
...
class ExpDecayLR(object):
    def __init__(self, optimizer, args, start_iter):
        ...
        self.lr_scheduler = ExponentialLR(
            optimizer, gamma=gamma, last_epoch=schedule_start
        )
    ...
    def compute_gamma(self, args):
        last_iter, start_iter = (
            args["train.max_iter"],
            args["train.exp_decay_start_iter"],
        )
        start_lr, last_lr = (
            args["train.learning_rate"],
            args["train.exp_decay_final_lr"],
        )
