...
def get_model(num_classes, args):
    ...
    if "pnf" in model_name:
        ...
        base_network_path = os.path.join(
            args["source"], "weights", base_network_name, "model_best.pth.tar"
        )
        model_fn = partial(
            resnet18, dropout=dropout_rate, pretrained_model_path=base_network_path
        )
    ...
    model = model_fn(
        classifier=train_classifier, num_classes=num_classes, global_pool=False
    )
    model.to(device)
    return model
...
def get_optimizer(model, args, params=None):
    ...
    if optimizer == "adam":
        ...
        optimizer = torch.optim.SGD(
            ...
            lr=learning_rate,
            momentum=0.9,
            nesterov=args["train.nesterov_momentum"],
    ...
    def embed_many(images, return_type="dict", kd=False, logits=False):
        with torch.no_grad():
            ...
            for name, extractor in extractors.items():
                if logits:
                    if kd:
                        all_logits[name], all_features[name] = extractor(
                            images[name], kd=True
                        )
                    else:
...
def get_pnf_extractor(trainsets, dataset_models, args):
    ...
    for dataset_name in trainsets:
        ...
        film_layers[dataset_name] = {
            k: v for k, v in state_dict.items() if "cls" not in k
        }
    ...
    base_extractor.eval()
    base_layers = {
        k: v for k, v in base_extractor.get_state_dict().items() if "cls" not in k
    }
