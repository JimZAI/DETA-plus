...
class MetaDatasetReader(object):
    def __init__(self, mode, train_set, validation_set, test_set):
        assert (
            train_set is not None or validation_set is not None or test_set is not None
        )
    ...
    def build_class_to_identity(self):
        ...
        self.dataset_name_to_dataset_id = {
            v: k for k, v in self.dataset_id_to_dataset_name.items()
        }
...
class MetaDatasetEpisodeReader(MetaDatasetReader):
    ...
    def __init__(
        ...
        super(MetaDatasetEpisodeReader, self).__init__(
            mode, train_set, validation_set, test_set
        )
        self.mode = mode
        ...
        # varied-way-10-shot
            ...
            if mode == "test":
                ...
                for item in test_set:
                    if item == "omniglot":
                        test_episode_desscription = config.EpisodeDescriptionConfig(
                            None, Shot, None, ignore_bilevel_ontology=False
                        )
                    else:
                        test_episode_desscription = config.EpisodeDescriptionConfig(
                            None, Shot, None, min_examples_in_class=20
                        )
                    next_task = self._init_single_source_dataset(
                        item, SPLIT_NAME_TO_SPLIT["test"], test_episode_desscription
                    )
...
class MetaDatasetBatchReader(MetaDatasetReader):
    ...
    def __init__(self, mode, train_set, validation_set, test_set, batch_size):
        super(MetaDatasetBatchReader, self).__init__(
            mode, train_set, validation_set, test_set
        )
