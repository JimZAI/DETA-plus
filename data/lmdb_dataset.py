...
class LMDBDataset:
    ...
    def __init__(
        self, extractor_domains, datasets, backbone, mode, dump_name, limit_len=None
    ):
        self.mode = mode
        ...
        for evalset in datasets:
            all_names = os.listdir(
                os.path.join(META_DATA_ROOT, "Dumps", backbone, mode, evalset)
            )
            self.dataset_readers[evalset] = [
                DatasetReader(extractor_domains, evalset, backbone, mode, name)
                for name in all_names
                if dump_name in name
            ]
        self._current_sampling_dataset = datasets[0]
        self.full_len = sum(
            [len(ds) for ds in self.dataset_readers[self._current_sampling_dataset]]
        )
    ...
    def __getitem__(self, idx):
        if self.mode == "train":
            random_lmdb_subset = random.sample(
                self.dataset_readers[self._current_sampling_dataset], 1
            )[0]
    ...
    def _load_db(self, info, class_id):
        ...
        self._env = lmdb.open(
            self._path, readonly=True, lock=False, readahead=False, meminit=False
        )
        ...
        if class_id is None:
            ...
            if os.path.isfile(cache_file):
                ...
                with self._env.begin(write=False) as txn:
                    self.keys = [key.decode("ascii") for key, _ in tqdm(txn.cursor())]
...
class DatasetReader(object):
    ...
    def __getitem__(self, idx):
        sample = dict()
        support_labels = pkl.loads(
            self._txn.get(f"{idx}_labels_support".encode("ascii"))
        )
        ...
        for dataset in self.trainsets:
            support_batch = pkl.loads(
                self._txn.get(f"{idx}_{dataset}_support".encode("ascii"))
            )
            query_batch = pkl.loads(
                self._txn.get(f"{idx}_{dataset}_query".encode("ascii"))
            )
