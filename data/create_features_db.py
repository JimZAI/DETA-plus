...
class DatasetWriter(object):
    ...
    def init(self):
        ...
        self.embed_many = get_domain_extractors(
            self.trainsets, self.dataset_models, self.args
        )
    ...
    def encode_dataset(self, n_tasks=1000):
        ...
        with tf.compat.v1.Session(config=config) as session:
            for idx in tqdm(range(n_tasks)):
                ...
                support_labels = SerializableArray(
                    sample["context_labels"].detach().cpu().numpy()
                )
                query_labels = SerializableArray(
                    sample["target_labels"].detach().cpu().numpy()
                )
                ...
                for dataset in support_embed_dict.keys():
                    ...
                    SerializableArray.__module__ = "utils"
                    txn.put(
                        f"{idx}_{dataset}_support".encode("ascii"),
                        pkl.dumps(support_batch),
                    )
                    txn.put(
                        f"{idx}_{dataset}_query".encode("ascii"), pkl.dumps(query_batch)
                    )
                    self._keys.extend(
                        [f"{idx}_{dataset}_support", f"{idx}_{dataset}_query"]
                    )
                txn.put(
                    f"{idx}_labels_support".encode("ascii"), pkl.dumps(support_labels)
                )
